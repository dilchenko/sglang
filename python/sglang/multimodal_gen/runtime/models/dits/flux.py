...
class FluxSingleTransformerBlock(nn.Module):
    ...
    def _forward_pre_attn_graphable(
        ...
        residual = joint_hidden_states
        norm_hidden_states, gate = self.norm(joint_hidden_states, emb=temb)
        if self.use_nunchaku_structure:
        ...
        return residual, norm_hidden_states, gate, mlp_hidden_states
    ...
    def forward(
        ...
        if (
            ...
            residual, norm_hidden_states, gate, mlp_hidden_states = (
                self._piecewise_cuda_graph_runner.replay_or_capture(
                    "flux_single_pre_attn",
...
class FluxTransformerBlock(nn.Module):
    ...
    def _forward_post_attn_graphable(
        ...
        ip_attn_output: torch.Tensor | None,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if isinstance(ip_attn_output, torch.Tensor):
